...
export const useOccupancyReport = (propertyId, limit = 30, autoGenerate = true) => {
  ...
  // Memoize fetchData với useCallback
  const fetchData = useCallback(async () => {
    try {
      ...
      // Nếu chưa có dữ liệu và cho phép auto-generate, tự động tạo báo cáo
  ...
  useEffect(() => {
    ...
    const debouncedRefetch = () => {
      ...
      refetchTimeout = setTimeout(async () => {
        ...
        // Tự động regenerate báo cáo khi có thay đổi (chỉ khi có propertyId)
        if (propertyId) {
          try {
            const today = new Date().toISOString().split("T")[0];
            await reportService.generateOccupancyReport(propertyId, today);
            
            // Fetch lại sau khi regenerate
            ...
            // Nếu regenerate fail, chỉ refresh data hiện có
            if (isMounted) {
              fetchData();
            }
          }
    ...
    console.log("🔔 Setting up realtime subscription for occupancy report");
    ...
    if (propertyId) {
      channel
        .on(
          ...
          (payload) => {
            console.log("🔔 REALTIME: Room created, refreshing occupancy report");
            debouncedRefetch();
          }
        )
        .on(
          ...
          (payload) => {
            console.log("🔔 REALTIME: Room updated, refreshing occupancy report");
            debouncedRefetch();
          }
        );
    } else {
      channel
        .on(
          ...
          (payload) => {
            console.log("🔔 REALTIME: Room created (all properties), refreshing occupancy report");
            debouncedRefetch();
          }
        )
        .on(
          ...
          (payload) => {
            console.log("🔔 REALTIME: Room updated (all properties), refreshing occupancy report");
            debouncedRefetch();
          }
        );
    }
    ...
    channel
      .on(
        ...
        (payload) => {
          console.log("🔔 REALTIME: Contract created, refreshing occupancy report");
          debouncedRefetch();
        }
      )
      .on(
        ...
        (payload) => {
          console.log("🔔 REALTIME: Contract updated, refreshing occupancy report");
          debouncedRefetch();
        }
      );
    ...
    channel
      .on(
        ...
        (payload) => {
          console.log("🔔 REALTIME: Tenant created, refreshing occupancy report");
          debouncedRefetch();
        }
      )
      .on(
        ...
        (payload) => {
          console.log("🔔 REALTIME: Tenant updated, refreshing occupancy report");
          debouncedRefetch();
        }
      );
    ...
    if (propertyId) {
      channel
        .on(
          ...
          (payload) => {
            console.log("🔔 REALTIME: New occupancy summary created");
            fetchData();
          }
        )
        .on(
          ...
          (payload) => {
            console.log("🔔 REALTIME: Occupancy summary updated");
            fetchData();
          }
        );
    } else {
      channel
        .on(
          ...
          (payload) => {
            console.log("🔔 REALTIME: New occupancy summary created (all properties)");
            fetchData();
          }
        )
        .on(
          ...
          (payload) => {
            console.log("🔔 REALTIME: Occupancy summary updated (all properties)");
            fetchData();
          }
        );
    }
    ...
    channel.subscribe((status) => {
      if (status === "SUBSCRIBED") {
        console.log("✅ Occupancy report realtime connected");
      } else if (status === "CHANNEL_ERROR") {
        console.error("❌ Occupancy report realtime error");
      }
    });

    // Cleanup subscription on unmount
    return () => {
      console.log("🔴 Cleaning up occupancy report realtime subscription");
      isMounted = false;
      clearTimeout(refetchTimeout);
